<template>
  <b-form @submit="onSubmit" @reset="onReset" v-if="show" id="modalInfo">
    <b-row class="mb-3">
      <b-col md="4">
        <image-upload :id="JSON.parse(industriesInfo.content).id" @image="addImage"></image-upload>
      </b-col>
      <b-col md="8">
          ...
          <b-row>
            <b-col sm="12">
              <b-form-group label="Title:">
                <b-form-input
                  type="text"
                  :value="JSON.parse(industriesInfo.content).title"
                  required
              ...
              <b-form-group label="Subtitle:">
                <b-form-input
                  type="text"
                  :value="JSON.parse(industriesInfo.content).subtitle"
                  required
    ...
    <b-row>
      <b-col>
        <b-form-group label="Content:">
          <text-editor :value="JSON.parse(industriesInfo.content).content"></text-editor>
        </b-form-group>
...
<script>
  ...
  export default {
    name: "UpdateIndustriesArticle",
    components: {
      'text-editor': TextEditor,
      'image-upload': ImageUpload
    },
    props: ['industriesInfo'],
    data() {
      return {
        form: {
